#ifndef DT_PROVENANCE_INTERCEPTION_OLLAMA_TASKS_H_
#define DT_PROVENANCE_INTERCEPTION_OLLAMA_TASKS_H_
...
#include "autogen/intercept_ollama_methods.h"
...
struct CreateParams {
  ...
  void LoadConfig(const clio::run::PoolConfig& pool_config) {
    // pool_config.config_ is the entire pool YAML node (all keys including
    // mod_name, pool_name, etc.). upstream_base_url may appear as a top-level
    // key OR be nested inside a "config: |" literal block sub-YAML.
...
/**
 * InterceptAndForwardTask — forward a request to Ollama and capture the
 * interaction
 */
...
#endif  // DT_PROVENANCE_INTERCEPTION_OLLAMA_TASKS_H_
